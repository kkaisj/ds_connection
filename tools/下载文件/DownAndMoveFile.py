...
class DownloadFile:
    ...
    def is_down_file_over(self):
        ...
        while wait > 0:
            ...
            if self.new_file_cnt > 0:
                time.sleep(2)
                file_str = "".join(os.listdir(self.download_dir_path)) # 再检查文件名
                if ".crdownload" not in file_str and ".tmp" not in file_str:
    ...
    def move_file(self, args):
        ...
        for idx, old_file_path in enumerate(self.get_file_path()):
            ...
            # 抖店下载失败的文件名是 数据.xlsx
            if file_name == "数据.xlsx":
                raise Exception("抖店导出数据失败，文件为无效文件, 请稍后等待任务重试.")
...
def main(args):
    ...
    # 是否允许下载多文件弹窗
    if xbot_visual.win32.window.contains_element(window="0", content_type="contains_element", element=package.selector("下载多个文件_允许"), _block=("main", 14, "IF 窗口包含")):
        xbot_visual.win32.element.click(window="0", element=package.selector("下载多个文件_允许"), clicks="click", button="left", keys="null", delay_after="1", anchor_type="center", sudoku_part="MiddleCenter", offset_x="0", offset_y="0", move_mouse=True, simulate=True, timeout="20", _block=("main", 15, "点击元素(win)"))
